  ...
  return (
    <Card className="border-zinc-100 bg-white p-6 rounded-xl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-zinc-900 mb-0.5">Brand Logos</h3>
          <p className="text-xs text-zinc-500">Brands you&apos;ve worked with</p>
        </div>
        <Button
          type="button"
          variant="outline"
      ...
      <div className="space-y-3">
        {fields.fields.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center border border-dashed border-zinc-200 rounded-lg bg-zinc-50/50">
            <Building2 className="h-8 w-8 text-zinc-300 mb-2" />
            <p className="text-xs text-zinc-500">No brand logos yet</p>
          </div>
        ) : (
          fields.fields.map((field, index) => (
            <div key={field.id} className="flex gap-2 p-3 border border-zinc-100 rounded-lg bg-zinc-50/50">
              <div className="flex-1 space-y-1">
                ...
                <Input
                  {...register(`brand_logos.${index}.url` as Path<T>)}
